    ...
    <table class="table table-border table-bordered table-bg">
        ...
        @foreach($cars as $car)
            <tr data-id="{{ $car->id }}">
                <td>{{$car->id}}</td>
                <td>{{ $car->name }}</td>
                <td><img src="{{ $car->logo }}" alt="" width="100px" height="100px"></td>
                <td>{{$car->brand}}</td>
                <td>{{$car->class}}</td>
                <td id="sc">{{$car->status==0?'空闲':'繁忙'}}</td>
                <td class="td-manage"><a title="编辑" href="javascript:;" onclick="car_edit('车辆编辑','{{ route('cars.edit',['car'=>$car]) }}','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> @if($car->status==0)
                    ...
                    @endif<a title="删除" href="javascript:;" onclick="car_del(this,'{{ $car }}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
            </tr>
        @endforeach
        </tbody>
    </table>
